...
Solution: SELECT facid, name, membercost, monthlymaintenance
FROM `Facilities`
...
Solution:SELECT name,monthlymaintenance,
CASE
...
Solution: SELECT firstname, surname AS lastname
FROM `Members`
WHERE memid = (
SELECT MAX( memid )
FROM Members );
...
Solution:
SELECT DISTINCT M.firstname member_name, F.name as facility
